        ...
        <div class="content">
          <div class="exportTo">
            <div class="optionList">
              <div
                :class="
                  `option ${index === selectedOption ? 'select' : ''} ${
...
export default class MindMap extends Vue {
  ...
  get svgClass() {
    return `stroke-width-${this.strokeWidth} ${
      this.spaceKey && this.zoomable ? "grab" : ""
    }`;
  }
  get optionTip() {
  ...
  makeNodeAdd(val: boolean) {
    const fObject = this.mindmapG.selectAll("foreignObject") as d3.Selection<
      Element,
      FlexNode,
      Element,
      FlexNode
    >;
    const gBtn = this.mindmapG.selectAll(".gButton") as d3.Selection<
      Element,
      FlexNode,
      Element,
      FlexNode
  ...
  makeContextMenu(val: boolean) {
    const selection = this.mindmapG.selectAll("foreignObject") as d3.Selection<
      Element,
      FlexNode,
      Element,
      FlexNode
  ...
  async fitContent() {
    ...
    await d3
      ...
      .then(() => {
        ...
        this.mindmapSvg
          .transition(this.easePolyInOut as any)
          .call(this.zoom.scaleTo, multiple);
      });
  }
  // 数据操作
  ...
  svgKeyDown() {
    ...
    if (event.metaKey) {
      ...
      // 针对节点的操作
      const sele = d3.select("#selectedNode") as d3.Selection<
        Element,
        FlexNode,
        Element,
        FlexNode
  ...
  updateNodeName() {
    // 文本编辑完成时
    const editP = document.querySelector(
  ...
  editNode(n: Element) {
    ...
    const fObj = d3
      ...
      .filter((a, b, c) => (c[b] as Element).parentNode === n) as d3.Selection<
      Element,
      FlexNode,
      Element,
      FlexNode
  ...
  focusNode(
    fObj: d3.Selection<Element, FlexNode, Element | null, FlexNode | undefined>
  ) {
    // 使节点处于可视区域
  ...
  editNew(newD: Mdata, depth: number, pNode: Element) {
    // 聚焦新节点
    d3.transition()
      .end()
      .then(
  ...
  fObjMousedown(d: FlexNode, i: number, n: ArrayLike<Element>) {
    ...
    if (edit && edit !== clickedNode) {
      const f = (d3
        .selectAll("foreignObject")
        .filter((d, i, n) => (n[i] as Element).parentNode === edit)
        .node() as Element).firstElementChild as HTMLElement;
  ...
  fObjectClick(d: FlexNode, i: number, n: ArrayLike<Element>) {
    // 两次单击进入编辑状态
    const sele = document.getElementById("selectedNode");
    const { dragFlag } = this;
    if (sele) {
      if (
        sele.getAttribute("__click__") === "1" &&
        n[i].parentNode === sele &&
  ...
  fObjectRightClick(d: FlexNode, i: number, n: ArrayLike<Element>) {
    ...
    console.log("fObjectRightClick");
    if (clickedNode.classList.contains("multiSelectedNode")) {
      const t: Mdata[] = [];
      (this.mindmapG.selectAll("g.multiSelectedNode") as d3.Selection<
        Element,
        FlexNode,
        Element,
        FlexNode
      ...
      const { data } = d;
      this.contextMenuItems[1].disabled = !(
  ...
  gBtnClick(a: FlexNode, i: number, n: ArrayLike<Element>) {
    // 添加子节点
    if ((n[i] as SVGElement).style.opacity === "1") {
      d3.event.stopPropagation();
      const d: FlexNode = d3
        .select(n[i].parentNode as Element)
        .data()[0] as FlexNode;
  ...
  mouseLeave(d: FlexNode, i: number, n: ArrayLike<Element>) {
    if ((n[i] as SVGElement).className.baseVal.includes("gButton")) {
      ...
      d3.selectAll("g.gButton")
        .filter((a, b, c) => (c[b] as Element).parentNode === n[i].parentNode)
        .style("opacity", 0);
    }
  }
  ...
  dragged(a: FlexNode, i: number, n: ArrayLike<Element>) {
    // 拖拽中【待完善】
    this.dragFlag = true;
    if (a.depth !== 0) {
      ...
      // 计算others的坐标
      (mindmapG.selectAll("g.node") as d3.Selection<
        Element,
        FlexNode,
        Element,
        FlexNode
      >)
        .filter(
          (d, i, n) => draggedNode !== n[i] && draggedNode.parentNode !== n[i]
        )
        .each((d, i, n) => {
          const gNode = n[i];
          const rect = {
            // 其他gRect的坐标，以及gRect的宽高
  ...
  multiSelect() {
    ...
    if (this.multiSeleFlag) {
      ...
      (mindmapG.selectAll("foreignObject") as d3.Selection<
        Element,
        FlexNode,
        Element,
        FlexNode
      >).each((d, i, n) => {
        ...
        const flag =
          pos.left < seleBox.right &&
          pos.bottom > seleBox.top &&
          pos.right > seleBox.left &&
          pos.top < seleBox.bottom;
        if (flag) {
  ...
  nest(d: FlexNode, i: number, n: ArrayLike<Element>) {
    ...
    (d3
      .select(n[i])
      .selectAll(
        `g${dd[0] ? `.depth_${dd[0].depth}.node` : ""}`
      ) as d3.Selection<Element, FlexNode, Element, FlexNode>)
      .data(dd, dKey)
      .join(appendNode, updateNode, exitNode);
  }
  appendNode(
    enter: d3.Selection<d3.EnterElement, FlexNode, Element, FlexNode>
  ) {
    const {
      $refs,
      expand,
    ...
    const foreign = gNode
      ...
      .attr("x", foreignX)
      .attr("y", foreignY);
    // const foreignDiv = foreign.append('xhtml:div').attr('contenteditable', false).attr('class', 'foreignContent').text((d: FlexNode) => d.data.name)
    const foreignDiv = foreign
      .append("xhtml:div")
      .attr("contenteditable", false)
      .attr("class", "foreignContent");
    // foreignDiv.html(node);
    // outerHTML
    foreignDiv
    ...
    foreignDiv.each((d, i, n) => {
      const observer = new ResizeObserver((l: any) => {
        ...
        console.log(t);
        foreign
          .filter((d: FlexNode, index: number) => i === index)
    ...
    const gBtn = gNode
      ...
      .style("visibility", gBtnVisible);
    gBtn
      .append("rect")
      .attr("width", gBtnSide)
      .attr("height", gBtnSide)
      .attr("rx", 3)
      .attr("ry", 3);
    // 小+号
  ...
  updateNode(update: d3.Selection<Element, FlexNode, Element, FlexNode>) {
    ...
    update.each((d, k, m) => {
      const node = d3.select(m[k]) as d3.Selection<
        Element,
        FlexNode,
        null,
        undefined
      ...
      const foreign = node
        .selectAll("foreignObject")
        .filter((d, i, n) => (n[i] as Element).parentNode === m[k])
