...
function initColor(d: Mdata, c?: string) {
  // 初始化颜色
  let color;
  if (d.id !== "0") {
...
class ImData {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (idArr.length >= 2) {
        ...
        if (delIndex && parent) {
          ...
          if (
            p === undefined ||
            p.id.split("-").length > parent.id.split("-").length
          ) {
            // 找出最高的parent
            p = parent;
          }
  ...
  add(id: string, child: Data) {
    // 添加新的子节点
    if (id.length > 0) {
      const parent = this.find(id);
      if (parent) {
        if ((parent._children?.length || 0) > 0) {
          // 判断是否折叠，如果折叠，展开
  ...
  move(delId: string, insertId?: string, i = 0) {
    // 节点在同层移动
    if (delId.length > 2) {
      if (!insertId) {
        // 左右转换
  ...
  reparent(parentId: string, delId: string) {
    // 节点移动到其他层
    if (delId.length > 2 && parentId.length > 0 && parentId !== delId) {
      ...
      if (delIndex && delParent && np) {
        ...
        if (del) {
          (np.children?.length || 0) > 0
            ? np.children?.push(del)
            : (np._children?.length || 0) > 0
            ? np._children?.push(del)
            : (np.children = [del]);

          initColor(
            del,
            parentId === "0" ? colorScale(`${(colorNumber += 1)}`) : np.color
          );
